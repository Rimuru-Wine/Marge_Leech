...
try:
    if ospath.exists('config.py'):
        import config
        for key, value in vars(config).items():
            if not key.startswith("__") and value is not None:
                environ.setdefault(key, str(value))
except Exception as e:
    log_error(f"Error loading config.py: {e}")

Interval = []
QbInterval = []
QbTorrents = {}
GLOBAL_EXTENSION_FILTER = ['!qB']
user_data = {}
extra_buttons = {}
list_drives_dict = {}
shorteners_list = []
...
if len(FFMPEG_CMDS) > 0:
    try:
        ...
        log_error(f"FFMPEG_CMDS error: {e}")
        FFMPEG_CMDS = {}
else:
    FFMPEG_CMDS = {}
...
if len(UPLOAD_PATHS) > 0:
    try:
        ...
        log_error(f"UPLOAD_PATHS error: {e}")
        UPLOAD_PATHS = {}
else:
    UPLOAD_PATHS = {}
...
PORT = environ.get('PORT') or BASE_URL_PORT
Popen(f"gunicorn web.wserver:app --bind 0.0.0.0:{PORT} --worker-class gevent", shell=True)

bot_cache['pkgs'] = ['qbittorrent-nox', 'ffmpeg', 'rclone', 'qbittorrent-nox|ffmpeg|rclone']

srun([bot_cache['pkgs'][0], "-d", f"--profile={getcwd()}"])
