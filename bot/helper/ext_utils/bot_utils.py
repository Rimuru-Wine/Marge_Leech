...
from bot import OWNER_ID, bot_name, bot_cache, DATABASE_URL, LOGGER, get_client, download_dict, download_dict_lock, botStartTime, user_data, config_dict, bot_loop, extra_buttons, user
...
def get_all_versions():
    try:
        ...
        vp = ''
    try:
        result = srun([bot_cache['pkgs'][1], '-version'], capture_output=True, text=True)
    ...
    try:
        ...
        vr = ''
    try:
        result = srun(['curl', '--version'], capture_output=True, text=True)
        vcl = result.stdout.split('\n')[0].split(' ')[1]
    except FileNotFoundError:
        vcl = ''
    try:
        ...
        try:
            ...
            vpy = "2.xx.xx"
    bot_cache['eng_versions'] = {'p7zip':vp, 'ffmpeg': vf, 'rclone': vr, 'curl': vcl,
...
class EngineStatus:
    def __init__(self):
        ...
        self.STATUS_CURL = f"curl {version_cache['curl']}"
    ...
    try:
        bot_cmds = [
            ...
            BotCommand(
                ...
                'Select files to download only torrents/magnet qbittorrent',
            ),
            BotCommand(
